...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
        ...
        mappings = configReq.ok ? await configReq.json() : { "dalban": "lp-eqk" };
    } catch (e) { mappings = { "dalban": "lp-eqk" }; }
    ...
    // 2. PROXY FETCH (FIXED FOR NAV-TITLE) (4).js]
    const proxyRequest = new Request(targetUrl, request);
    
    // Log: Set Host target biar Pages nggak bingung
    proxyRequest.headers.set("Host", targetHostname);
    
    // Log: Kirim hostname asli buat dibaca id.js di nav-title (4).js]
